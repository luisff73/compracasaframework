...
class login_bll
{
	...
	public function get_register_BLL($args)
	{
		...
		$hashavatar = md5(strtolower(trim($args[3])));
		...
		if ($check) {
			echo json_encode("error_email_reg");
			exit;
		} else {
			try {
				$check_user = $this->dao->check_user($this->db, $args[0]);
			} catch (Exception $e) {
				echo json_encode("error");
				exit;
			}
			...
			if ($check_user) {
				echo json_encode("error_user_exist");
				exit;
			} else {
				try {
				...
				$message = ['type' => 'validate', 'token' => $token_email, 'toEmail' =>  $args[3]];
	...
	public function get_verify_email_BLL($args)
	{
		...
		error_log('Valor de $args en get_verify_email_BLL: ' . print_r($args, true));
		//return "hola verify";
		$result = 'fail';
	...
	public function get_recover_email_BLL($email)
	{
		//echo 'hola';
